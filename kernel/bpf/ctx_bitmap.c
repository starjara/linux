	...
	const struct sk_buff *kernel_ctx, struct __sk_buff *bpf_ctx)
{
	...
	for (bit = 0; bit < SOCKET_FILTER_BITMAP_SIZE; bit++) {
		...
		case offsetof(struct __sk_buff, vlan_present):
			bpf_ctx->vlan_present = kernel_ctx->vlan_all != 0 ? 1 : kernel_ctx->vlan_all;
			break;
		...
		case offsetof(struct __sk_buff, tc_index):
			bpf_ctx->tc_index = kernel_ctx->tc_index;
			break;
		case offsetof(struct __sk_buff, napi_id):
			bpf_ctx->napi_id = kernel_ctx->napi_id;
			if (bpf_ctx->napi_id < MIN_NAPI_ID)
				bpf_ctx->napi_id = 0;
			break;
		case offsetof(struct __sk_buff, cb[0]) ... offsetofend(struct __sk_buff, cb[4]) - 1:
		...
		case offsetof(struct __sk_buff, local_ip4):
			...
			break;
		case offsetof(struct __sk_buff, remote_ip6[0]) ... offsetof(struct __sk_buff, remote_ip6[3]):
			memcpy(bpf_ctx->remote_ip6, skc->skc_v6_daddr.s6_addr32, 4*sizeof(__u32));
			break;
		case offsetof(struct __sk_buff, local_ip6[0]) ... offsetof(struct __sk_buff, local_ip6[3]):
			memcpy(bpf_ctx->local_ip6, skc->skc_v6_rcv_saddr.s6_addr32, 4*sizeof(__u32));
			break;
	...
	struct bpf_perf_event_data *bpf_ctx)
{
	...
	for (bit = 0; bit < PERF_EVENT_BITMAP_SIZE; bit++) {
		...
		case offsetof(struct bpf_perf_event_data, regs):
			memcpy(&bpf_ctx->regs, kernel_ctx->regs, sizeof(bpf_user_pt_regs_t));
			break;
		}
	}
}
