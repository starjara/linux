...
/* Garden Start : Including Header Files */
...
#define MAP_BITMAP_SIZE 32 // Garden
...
static void __build_epilogue(bool is_tail_call, struct rv_jit_context *ctx, bool is_sandboxed)
{
	...
	if (is_sandboxed) {
		emit_ld(RV_REG_SP, 0, RV_REG_S11, ctx);
	}
	 End of Garden */
...
/* Garden Start : emit_sfi */

static u8 emit_sfi(u8 rs, int off, struct rv_jit_context *ctx, bool is_map)
{
...
//		}
	...
	case BPF_LDX | BPF_PROBE_MEM | BPF_DW:
	{
		...
		} else if (is_map_reg(ctx->prog, rd) && ctx->prog->map_info->current_active_map) {
			bitmap_clear(ctx->prog->map_info->map_reg_bitmap, rd, 1);
	...
	case BPF_ST | BPF_MEM | BPF_H:
		emit_imm(RV_REG_T1, imm, ctx);
		if (is_12b_int(off) && !is_sandboxed) {
...
*/	
	...
	case BPF_STX | BPF_MEM | BPF_DW:
	{
		...
		case BPF_W:
			...
			break;
		case BPF_DW:
			emit(rv_sd(masked_addr_reg, 0, rs), ctx);
			break;
		}
		break;
	} 
